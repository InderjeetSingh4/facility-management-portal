...
// 2. Admin/Manager: Approve complaint
...
// 3. Admin/Manager: Reject complaint (resets back to open for worker to fix)
...
export async function getComplaints() {
  ...
  return complaints.map((c: any) => ({
    ...
    status: (c.status || (c.is_resolved ? 'approved' : 'open')) as string,
